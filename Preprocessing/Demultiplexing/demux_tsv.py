...
def load_whitelist(whitelist_file):
    ...
    Expects a TSV file with columns: well_name, well_number, barcode
    """
    mapping = {}
    ...
    logging.info(f"Loaded {len(mapping)} barcodes from whitelist TSV")
    return mapping
...
def main():
    parser = argparse.ArgumentParser(
        description="Demultiplex FASTQ using a barcode whitelist TSV with optional mismatch tolerance.",
        formatter_class=argparse.RawTextHelpFormatter
    )
    parser.add_argument("fastq", help="Input FASTQ file with barcoded reads.")
    parser.add_argument("whitelist", help="Whitelist TSV with columns: well_name, well_number, barcode.")
    parser.add_argument("--outdir", default='.', help="Directory to write demultiplexed FASTQs and report.")
    parser.add_argument(
        "--mismatch",
        type=int,
        default=0,
        help=(
            "Maximum Hamming mismatches for barcode matching (default: 0)."
        )
    )
    ...
    report_fn = os.path.join(args.outdir, f"{os.path.splitext(os.path.basename(args.fastq))[0]}_other_demux_report.txt")
